...
## ListUserTasks
You can execute the `ListUserTasks` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useListUserTasks(dc: DataConnect, vars: ListUserTasksVariables, options?: useDataConnectQueryOptions<ListUserTasksData>): UseDataConnectQueryResult<ListUserTasksData, ListUserTasksVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useListUserTasks(vars: ListUserTasksVariables, options?: useDataConnectQueryOptions<ListUserTasksData>): UseDataConnectQueryResult<ListUserTasksData, ListUserTasksVariables>;
```

### Variables
The `ListUserTasks` Query requires an argument of type `ListUserTasksVariables`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
...
```
### Return Type
Recall that calling the `ListUserTasks` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListUserTasks` Query is of type `ListUserTasksData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ListUserTasks`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, ListUserTasksVariables } from '@dataconnect/generated';
import { useListUserTasks } from '@dataconnect/generated/react'

export default function ListUserTasksComponent() {
  // The `useListUserTasks` Query hook requires an argument of type `ListUserTasksVariables`:
  const listUserTasksVars: ListUserTasksVariables = {
    userId: ..., 
  };

  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useListUserTasks(listUserTasksVars);
  // Variables can be defined inline as well.
  const query = useListUserTasks({ userId: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useListUserTasks(dataConnect, listUserTasksVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useListUserTasks(listUserTasksVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useListUserTasks(dataConnect, listUserTasksVars, options);
...
## ToggleTask
You can execute the `ToggleTask` Mutation using the `UseMutationResult` object returned by the following Mutation hook function (which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts)):
```javascript
useToggleTask(options?: useDataConnectMutationOptions<ToggleTaskData, FirebaseError, ToggleTaskVariables>): UseDataConnectMutationResult<ToggleTaskData, ToggleTaskVariables>;
```
You can also pass in a `DataConnect` instance to the Mutation hook function.
```javascript
useToggleTask(dc: DataConnect, options?: useDataConnectMutationOptions<ToggleTaskData, FirebaseError, ToggleTaskVariables>): UseDataConnectMutationResult<ToggleTaskData, ToggleTaskVariables>;
```

### Variables
The `ToggleTask` Mutation requires an argument of type `ToggleTaskVariables`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
...
```
### Return Type
Recall that calling the `ToggleTask` Mutation hook function returns a `UseMutationResult` object. This object holds the state of your Mutation, including whether the Mutation is loading, has completed, or has succeeded/failed, among other things.

To check the status of a Mutation, use the `UseMutationResult.status` field. You can also check for pending / success / error status using the `UseMutationResult.isPending`, `UseMutationResult.isSuccess`, and `UseMutationResult.isError` fields.

To execute the Mutation, call `UseMutationResult.mutate()`. This function executes the Mutation, but does not return the data from the Mutation.

To access the data returned by a Mutation, use the `UseMutationResult.data` field. The data for the `ToggleTask` Mutation is of type `ToggleTaskData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ToggleTask`'s Mutation hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, ToggleTaskVariables } from '@dataconnect/generated';
import { useToggleTask } from '@dataconnect/generated/react'

export default function ToggleTaskComponent() {
  // Call the Mutation hook function to get a `UseMutationResult` object which holds the state of your Mutation.
  const mutation = useToggleTask();

  // You can also pass in a `DataConnect` instance to the Mutation hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const mutation = useToggleTask(dataConnect);

  // You can also pass in a `useDataConnectMutationOptions` object to the Mutation hook function.
  const options = {
    onSuccess: () => { console.log('Mutation succeeded!'); }
  };
  const mutation = useToggleTask(options);
  ...
  const mutation = useToggleTask(dataConnect, options);

  // After calling the Mutation hook function, you must call `UseMutationResult.mutate()` to execute the Mutation.
  // The `useToggleTask` Mutation requires an argument of type `ToggleTaskVariables`:
  const toggleTaskVars: ToggleTaskVariables = {
    id: ..., 
    completed: ..., 
  };
  mutation.mutate(toggleTaskVars);
  // Variables can be defined inline as well.
  ...
  mutation.mutate(toggleTaskVars, options);
